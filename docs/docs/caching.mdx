...
The main tunning point of a LRU is its size in terms of memory allocation or number of items cached. To bound the cache
based on memory size, use `read-cache.size` [configuration](./configuration):
...
For the second type of issue, we pre-compute expand subgraph in a separate process and load the subgraph as the cache
into Redis.

There will be 2 types of loading mechanisms:

1. Spring Scheduler for medium-sized cache generation
2. External Process for large cache computations
...
### Spring Scheduler Loading

#### Enabling the Scheduler

The scheduler is disabled by default. To enable it, set the `spring-scheduler.enabled` to `true` in
...
At this moment, only a cron regex is supported that controls how often the scheduler is run. For example, to trigger a
cache load on 1:01 am every day:
